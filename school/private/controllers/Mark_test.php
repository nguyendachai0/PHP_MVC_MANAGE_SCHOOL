...
class Mark_test extends Controller
{
    public function index($id = '', $user_id = '')
    {
        ...
        // set as marked
        if (isset($_GET['set_marked']) &&  (get_mark_percentage($row->test_id, $user_id) >= 100)) {


            $query = "update answered_tests set marked = 1, marked_by = :marked_by, marked_date = :marked_date,score = :score where test_id = :test_id && user_id =:user_id limit 1";
            $tests->query($query, [
                'test_id' => $id,
                'user_id' => $user_id,
