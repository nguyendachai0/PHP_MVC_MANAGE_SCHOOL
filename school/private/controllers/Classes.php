...
class Classes extends Controller
{
    function index()
    {
        ...
        if (Auth::access('admin')) {
            ...
            if (isset($_GET['find'])) {
                $find = '%' . trim($_GET['find']) . '%';
                $query = "select classes.class, {$mytable}. * from $mytable join classes on classes.class_id = {$mytable}.class_id where ({$mytable}.user_id = :user_id && disabled = 0 && classes.class like :find && year(classes.date) = :school_year)";
                $arr['find']  = $find;
            }
